...
上面那些生态故事都是别人的。真正让这份契约显得紧迫的,是回头看我们自己的匿名历史 token 账本,发现同样的形状正盯着我们。下面都是来自生产运行的聚合窗口——已匿名,没有任何值得引用的会话身份——但模式足够一致,值得拿出来看。

反复出现的症状是一种**缓存 token 平台期**:一长串连续调用报告着*完全相同*的为正缓存 token 数,而它周围的输入 token 却随着对话增长而往上漂。我们抽出的几个窗口:

| 账本窗口(匿名) | 连续调用数 | 缓存 token(保持不动) | 周围的输入 token | 是否记录了缓存身份? |
| --- | --- | --- | --- | --- |
| 某个生产窗口 | 136 | 33,280 | 从约 53k 涨到约 55k+ | 无 session/thread/prompt-cache-key |
| 另一段长跑 | 68 | 99,328 | 约 149k–151k | 无 prompt-cache-key |
| 一段更晚、已插桩的运行 | 30 | 194,048 | (唤醒式调用) | 有相同 session id + thread id;**prompt cache key 为 null/缺失** |

前两个窗口是最纯粹的「之前」世界:几十到上百次调用钉在同一个为正缓存 token 数上,却**完全没有**记录任何缓存身份元数据。缓存显然在命中*某个东西*——数字为正且稳定——但热前缀并没有随对话增长而增长,而账本里没有任何东西能让我们判断:这份稳定到底是有意的亲和性,还是一个意外变馊的缓存桶。光凭原始缓存命中数,根本回答不了这个问题。

第三个窗口更让人不安,因为它出现在我们*已经*开始多记录一些之后。这里我们终于能看到一个稳定的 session id 和 thread id 横跨 30 次连续的唤醒式调用——这是真实可观测的、请求头层面的亲和性——但 body 里的 `prompt_cache_key` 在记录下来的元数据里仍然是 null/缺失。于是我们能看到请求头是稳定的,却无法证明 body 键是和它们一起被携带的;而且即便平台期长到这种程度,也没有出现任何轮换事件——可一段这么长的平台期,恰恰是一个保守的「N 次相同命中后轮换」策略本该抓住的。(其他已插桩的窗口显示了同样的缺口:约 50 次连续调用平在 79,872,约 46 次平在 52,736,每个都能看到 session/thread,却都缺 prompt 键。)

这就是「之前」:**缓存 token 平台期,却没有可靠的缓存身份。** 有时根本没有身份;有时请求头稳定却没记录 body 键;每一种情况下,都无从区分一份健康的热缓存和一份悄悄变馊的缓存,平台期拖长时也没有任何轮换信号。

我们想要的「之后」是一份契约,而不是一个指望。我们意图中的修复后信号是:每一次 Codex 请求都在三个地方同时携带*同一个*当前亲和 id——body 的 `prompt_cache_key`、`session-id` 请求头、`thread-id` 请求头——并且同一个 id 也写进用量元数据,好让仅凭账本就能审计。如果缓存 token 读数在一段有上限的为正命中里保持完全相同(我们的策略用 8 次),当前 id 就持久轮换,并发出一个 `codex_cache_affinity_rotated` 事件,后续调用沿用新的 id。这把旧的平台期从「神秘的成本行为」变成了「带有上限逃生口的可观测状态」。

把已定与未定说清楚:这些账本窗口*正是*那种促使我们同时加上身份不变式与轮换可观测性的证据——它并不是在宣称每一条线上路径都已经修好。事实上,正是那个浮现出第三个窗口的线上调查,才让我们现在要求 body 键与两个请求头被*一起*记录下来:这样下次再出现这样的平台期时,账本能用一行字回答——契约到底有没有守住。
